...
卡内基梅隆大学（Carnegie Mellon University）匹兹堡校区及周边街区的交互式三维复刻，基于 three.js，在浏览器中运行。
覆盖范围包括 CMU 主校区、匹兹堡大学与 Oakland 文化区、Schenley Park，以及 Shadyside（Walnut St、Kenmawr）与 East Liberty（Penn Ave）。
...
- **地标建模**：参照实景照片手工建模 50 余座建筑与地标，包括 Hamerschlag Hall、Hunt Library、Wean Hall、Gates-Hillman Center、Cohon University Center、Tepper Quad、The Fence、Cathedral of Learning、Kenmawr Apartments 与 East Liberty Presbyterian Church 等。
- **三种浏览模式**：俯瞰、第一人称步行（含碰撞检测与地形跟随）、自由飞行。
- **导航与信息**：建筑介绍面板、搜索、小地图、17 站导览，以及中英文路名标注与路口路牌。
